...
public interface EstacionamientoRepository extends JpaRepository<Estacionamiento, Long> {
    @Query("SELECT c FROM Estacionamiento c WHERE c.id NOT IN (" +
            ...
            ")" +
            ")")
    List<Estacionamiento> findAvailableCubiculos(@Param("dateInit") Date dateInit, @Param("dateEnd") Date dateEnd);
}
